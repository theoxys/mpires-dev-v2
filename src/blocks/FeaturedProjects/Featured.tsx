...
const FeaturedProjects = ({ projects }: { projects: FeaturedProject[] }) => {
  ...
  return (
    <section id="latestProjects" className="w-full flex flex-col gap-16px justify-between">
          ...
          <div className={infoStyle()}>
            ...
            <>
              {projects.map((project, i) => (
                ...
                >
                  <h3 className="whitespace-nowrap">{project.title}</h3>
                  <p className="text-subtle">{project.desc}</p>
                </div>
              ))}
            </>
...
const ProgressBar = ({ progress }: { progress: number }) => {
  const styles = cva(['w-0px h-2px', 'absolute bottom-0px left-0px z-10', 'bg-inverse'])
